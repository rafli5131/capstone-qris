...
export const options = {
  stages: [
    { duration: '30s', target: 500 },   // Peak rural: moderate concurrent users
    { duration: '120s', target: 500 },
    { duration: '30s', target: 0 },
  ],
  thresholds: {
    http_req_duration: ['p(95)<5000'], // Peak rural: high latency tolerance
...
export default function () {
  ...
  // Simulate peak rural network: high latency + moderate load
  sleep(Math.random() * 1 + 0.5); // 0.5-1.5s delay

  group('QRIS Inquiry - Peak Rural Network', () => {
  ...
  sleep(Math.random() * 1.5 + 0.5); // Additional rural delay

  // Create 5 payments and store their transaction IDs
  for (let i = 0; i < 5; i++) {
    group(`QRIS Payment ${i + 1} - Peak Rural Network`, () => {
    ...
    sleep(Math.random() * 0.8 + 0.4); // Rural delay between payments
  }
  ...
  transactionIds.forEach((transactionId, index) => {
    group(`Transaction Status ${index + 1} - Peak Rural Network - ${transactionId}`, () => {
    ...
    sleep(Math.random() * 0.3 + 0.1); // Small rural delay between status checks
  });
